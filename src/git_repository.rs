...
pub fn repo_path(gitdir: &PathBuf, paths: &[PathBuf]) -> PathBuf {
    paths.iter().fold(gitdir.clone(), |acc, p| acc.join(p))
}
...
pub fn repo_file(
    ...
    // Python: *path[:-1]
    // Rust: paths[..n].to_vec()
    if let Some(_) = repo_dir(gitdir, &paths[..paths.len() - 1], mkdir)? {
...
pub fn repo_dir(
    ...
    let path: PathBuf = repo_path(gitdir, paths);
...
pub fn repo_create(path: &PathBuf) -> Result<GitRepository> {
    ...
    repo_dir(&repo.gitdir, &[PathBuf::from("branches")], Some(true))?;
    repo_dir(&repo.gitdir, &[PathBuf::from("objects")], Some(true))?;
    repo_dir(
        &repo.gitdir,
        &[PathBuf::from("refs"), PathBuf::from("tags")],
        Some(true),
    )?;
    repo_dir(
        &repo.gitdir,
        &[PathBuf::from("refs"), PathBuf::from("heads")],
        Some(true),
    )?;

    // ["description"][:-1] は [] になって ./test/.git になるがそうなって欲しくない
    if let Some(path) = repo_file(&repo.gitdir, &[PathBuf::from("description")], Some(false))? {
        let mut f = OpenOptions::new().write(true).create(true).open(path)?;
        f.write(b"Unnamed repository; edit this file 'description' to name the repository.\n")?;
    }

    if let Some(path) = repo_file(&repo.gitdir, &[PathBuf::from("HEAD")], Some(false))? {
        let mut f = OpenOptions::new().write(true).create(true).open(path)?;
        f.write(b"ref: refs/heads/master\n")?;
    }

    if let Some(path) = repo_file(&repo.gitdir, &[PathBuf::from("config")], Some(false))? {
